...
function get_post_images($post_id=null) {
    ...
    foreach ($images as $index => $i) {
        ...
        $src = wp_get_attachment_image_src($i->ID, 'student_slide');
        $images[$index]->student_slide = $src[0];

        $src = wp_get_attachment_image_src($i->ID, 'gallery_large');
        $images[$index]->gallery_large = $src[0];

        $src = wp_get_attachment_image_src($i->ID, 'student_thumb');
        $images[$index]->student_thumb = $src[0];
	}
    return $images;
}
